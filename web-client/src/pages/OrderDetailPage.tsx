...
export default function OrderDetailPage() {
  ...
  if (error || !order) {
    return (
      <div className="text-center py-12">
        <p className="text-destructive mb-4">{error || 'Commande non trouvée'}</p>
        <button
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-lg px-4 py-2 rounded-full font-semibold ${getStatusColor(order.status)}`}>
            ...
            <div className="space-y-4">
              {['pending', 'confirmed', 'preparing', 'ready', 'delivered'].map((step, index) => (
                <div key={step} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                        ['pending', 'confirmed', 'preparing', 'ready', 'delivered'].indexOf(order.status) >= index
                  ...
                  <div className="pt-1">
                    <p className="font-semibold capitalize">{getStatusLabel(step)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              <p>
                ...
                <span className="font-semibold">
                  {order.deliveryType === 'delivery' ? 'Livraison à domicile' : 'Retrait sur place'}
                </span>
              </p>
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            {!['delivered', 'cancelled'].includes(order.status) && (
              <button
                ...
                className="w-full border border-destructive text-destructive py-2 rounded-lg hover:bg-destructive/10 transition font-semibold"
              >
                Annuler la commande
              </button>
            )}
