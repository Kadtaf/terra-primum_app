export const groupOcrLinesIntoProducts = (cleaned: string): string[] => {
    ...
    /adresse|facture|commande|livraison|référence client|total ht|total ttc|tva|page|easy|boulevard|rcs|ape/i.test(line);

    const isProductLine = (line: string) =>
    !isNoise(line) &&
    (hasReference(line) || hasDesignation(line) || hasQuantity(line) || hasPrice(line));
